...
class YoloDatasets(keras.utils.Sequence):
    def __init__(self, annotation_pairs, input_shape, anchors, batch_size, num_classes, anchors_mask, train):
        ...
        self.num_layers = len(self.anchors_mask)
        self.train = train
    ...
    def get_random_data(self, annotation_pairs, input_shape, max_boxes=100, jitter=.3, hue=.1, sat=1.5, val=1.5,
                        random=True):
    ...
    def preprocess_true_boxes(self, true_boxes):
        ...
        grid_shapes = [input_shape // {0: 32, 1: 16, 2: 8}[l] for l in range(self.num_layers)]
        # -----------------------------------------------------------#
        #   y_true -> [(m,13,13,3,85),(m,26,26,3,85),(m,52,52,3,85)]
        # -----------------------------------------------------------#
        y_true = [np.zeros((m, grid_shapes[l][0], grid_shapes[l][1], len(self.anchors_mask[l]), 5 + self.num_classes),
